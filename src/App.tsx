    ...
    return (
      removeDuplicates.map((standard_drinkware, id) => (
        <button
          ...
          key={id}
          className={`button ${selectedFilters?.includes(standard_drinkware) ? "bg-stone-500 p-3 transition-all" : "p-3 hover:bg-stone-400 transition-all"}`}
    ...
    <>
      ...
      <div className="flex flex-wrap justify-center">
        <input
          ...
          ref={inputRef}
          name="search"
          className="m-2 p-2 w-2/5 border border-stone-300 text-stone-700"
        />
      </div>
      <div className="flex flex-wrap justify-center">
      <div className="p-4 border border-stone-300 flex flex-wrap justify-center w-[80vw]">
      ...
      <div className="p-4 border border-stone-300 flex flex-wrap justify-center mt-2 w-[80vw]">
        <div className="text-center flex justify-center pb-4 w-full">
          <button onClick={() => setShowFilters(!showFilters)} className="hover:bg-stone-400 transition-all p-2">Filter by Glassware</button>
        </div>
        <div className="text-center flex justify-center w-full">
          {showFilters ? (
            <div className="border-t border-slate-200 m-auto text-center p-4">
              {setFilters()}
            </div>
          ) : null}
        </div>
